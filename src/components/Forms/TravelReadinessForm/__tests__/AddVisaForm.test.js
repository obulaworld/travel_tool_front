...
describe('<AddVisaForm />', () => {
  ...
  it('renders file selector', () => {
    wrapper.find('.visa-select-file');
    expect(wrapper.find('.visa-select-file').length).toEqual(1);
  });

  it('handles submit', () => {
    moxios.stubRequest(process.env.REACT_APP_CLOUNDINARY_API, { status: 200, data: { url: 'url' } });
    wrapper.state().file = validFile;
    wrapper.find('.visa-form').simulate('submit', event);
    moxios.wait(() => {
